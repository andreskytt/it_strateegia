...
\begin{frame}[fragile]
  \frametitle{IT Kulud}
	...
	\begin{itemize}
		...
		\note{Mitte lihtsalt väärtust, vaid väärtust aktsionäridele. Kes on aktsionär ja kuidas ta oma tahet väljendab?\\}
...
\begin{frame}[fragile]
  \frametitle{Arenduse ja halduse kulud}
	...
	\end{columns}
\end{frame}
\note{K-kulud kokku, H-halduskulud, A-arenduskulud mingis aastas\\Kuna IT väärtust tajutakse sageli just läbi uute arenduste järeldub siit, et kui vana ja uue arenduse suhe on vale, väheneb konstantsete investeeringute puhul IT võimekus uut tajutud väärtust tooota.}
...
\begin{frame}[fragile]
  \frametitle{Järeldused}
	\begin{itemize}
		...
		\note{Sellega mõistega kohtume natuke hiljem täpsemalt\\}
		\item Tuleb olla ettevaatlik investeeringute kogumisega
		\note{Investeeritu hooldamine, arendamine, kasutajatugi, varukoopiad jne. vajavad kõik raha\\}
	\end{itemize}
...
\begin{frame}[fragile]
	\frametitle{Arhitektuur ja äri}
			...
			\end{itemize}
\end{frame}
\note{Oluline rõhk võimekusel. Mäletate: arhitektuur määrab vektorid, mille sisustab disain. Kui aga parameetrid on paigast ära, ei tule kasumist midagi. Ehk. Vale arhitektuur tähendab, et äri ei saa olla kasumlik, õige arhitektuur ei tähenda, et äri on kasumlik.\par Black \& Deckeri platvormi näide seitsmekümnendatest, Volkswagen vs. GM}
...
\begin{frame}[fragile]
  \frametitle{Definitsiooni järelmid}
	\begin{itemize}
		...
		\note{Siit peaks tulema vihje rõhuasetuste kohta: enamasti keskendutakse programmeerimisele, muud jäävad unarusse\par}
		\item Ei ole viidet arendusprotsessile, kui sellisele
			\begin{itemize}
				...
				\note{Distipliinide kokku panemise parim viis sõltub olukorrast. Retsepti ei ole\par}
				\item Igapäevases elus saab arendusprotsess disproportsionaalselt palju tähelepanu
				\note{Sest kvarteti liikmeid ümber paigutada on lihtne, mõistliku testimisraamistiku välja töötamine on aga väga keeruline\par}
			\end{itemize}
	...
	\end{itemize}
\note{Programmeerijad on tarkvaratehnika kaaperdanud tõmmates tähelepanu endale ja jättes varju teised valdkonnad. Mis, muidugi, võtavad ära suurema osa ka programmeerija päevast kuid siiski kutsutakse neid 'programmeerijateks' ja mitte kuidagi teisiti}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{center}
\end{frame}
\note{Lugege Joeli, ta on hea. Suurem osa sisu veebis tasuta saadaval aga raamat on mugavam \par Muidugi on toodud vastuolu vaid üks võimalikke, kuid illustreerib seoseid kenasti. \par Veelkord: kuidas te tarkvara ehitate, nagu ka kõik muu, mida te teete, peab toetama äristrateegiat\\ Märkustes eraldi pikk jutt sellest, kuidas strateegia peab olema selgepiiriline ja otsuseid toetav}
  ...
  \begin{itemize}
	\item Läbi aja on üritatud hõbekuuli siiski leida
		  \begin{itemize}
			...
			\note{Väga levinud lähenemine avalikus sektoris, bürokraadid armastavad kõikvõimalikke dokumente toota\par}
...
\small  
	...
	\end{enumerate}
	\note{Taktikalised, kuid illustreerivad strateegilisi probleeme \par Osa neist on tänapäeval elementaarsed, kuid mitte kõik. Jällegi illustreerib kenasti eri distsipliinide olulisust kuigi kaldub programmeerijate poole\par Olulised: ehitamine, vigade parandamine, rahu/vaikus.\par Parimad raha eest ostetavad tööriistad on tänapäeval asendunud parimate raha eest saadavate konverentside/raamatutega. Inimese arendamise osas ei ole mõistlik koonerdada (vt. märkmed)}
\normalsize
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{itemize}
\note{Shipping is a feature. A really important feature. Your product must have it. Spolsky, duct tape programmer\\Äril on täpselt samasugused võlad. Ainult juhtimisalases kirjanduses seda väga ei puudutata.}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Tehnilise võla kvadrant}
	...
	\end{center}
	\cite{fowlerdebt}
	\note{On oluline aru saada, millisesse kvadranti teie võlg läheb. \par "pole aega": kõrge intressiga kiire laen riskantseteks projektideks \par "millised kihid?": sms laen \par "Peame tarnima": arvelduskrediit \par "nüüd teame": eluasemelaen. Suhteliselt vältimatu, sellest tuleb viimast võtta}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Inimesed käituvad viisil, mis \emph{maksimeerib nende huvisid}. Kuidas need ettevõtte huvidega sobivad, on juhi probleem.
	\begin{itemize}
		...
		\note{Küsimusi inimeste kohta enne poliitika rakendamist tuleb küsida nimeliselt!\par}
			\begin{itemize}
				...
				\note{Ülejäänud seltskond on kahtlemata olulised, kuid mitte muutuse kontekstis}
			\end{itemize}
...
\begin{frame}[fragile]
  \frametitle{Ressursid}
	...
	\end{itemize}
	\note{Lugege märkust it-strateegia ja äristrateegia seoste kohta. Mis on IT probleem ja kuidas ta on seotud äri omaga?}
\end{frame}
